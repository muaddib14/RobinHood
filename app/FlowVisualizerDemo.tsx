...
/**
 * Static illustrative example, always visible on the landing page —
 * shows what the real FlowVisualizer (fed by an actual /api/scan result,
 * rendered inside ScanShell once a user scans) looks like, without
 * requiring a scan first. Fixture data is fabricated on purpose to show
 * the full 5-layer / flagged-path shape a real trace can produce; it is
 * explicitly labeled "Example output" in the header so it's never
...
const NOW = 1784820000000; // fixed timestamp — avoids SSR/client hydration drift
...
const NODE_BY_ID: Record<string, DemoNode> = Object.fromEntries(NODES.map((n) => [n.id, n]));
...
export default function FlowVisualizerDemo() {
  ...
  const [sortKey, setSortKey] = useState<keyof DemoEdge | "source">("ts");
  ...
  const handlePointerMoveNode = (e: React.PointerEvent<SVGGElement>) => {
    ...
    setPos((prev) => ({ ...prev, [activeDrag.current!.id]: { x: nx, y: ny } }));
  };
  ...
  const handleHideTip = () => setTooltip((prev) => ({ ...prev, show: false }));

  const getBucketPos = (clientX: number) => {
  ...
  const handleTlDown = (e: React.PointerEvent) => {
    const v = getBucketPos(e.clientX);
    bStart.current = v;
  ...
  const handleTlMove = (e: React.PointerEvent) => {
    ...
    const p = getBucketPos(e.clientX);
  ...
  const handleTlUp = () => {
    ...
    setTlStart((s) => {
      if (tlEnd - s < 0.02) return 0;
      return s;
    });
    if (tlEnd - tlStart < 0.02) setTlEnd(1);
  };
  ...
  }, [visibleEdges, sortKey, sortDir]);

  const handleSort = (key: keyof DemoEdge | "source") => {
  ...
  const h0 = Math.round(72 - tlStart * 72);
  const h1 = Math.round(72 - tlEnd * 72);

  return (
    <div className="flow-vis-container reveal in">
      <div className="flow-head">
        <div className="eyebrow">Example output · not a live scan</div>
        <h2>
      ...
      <div className="toolbar">
        ...
        <button type="button" className={`chip ${flaggedOnly ? "on" : ""}`} onClick={() => setFlaggedOnly((v) => !v)}>
          Flagged paths only
        </button>
        <div className="ctrl">
          Min USD
          <input type="range" min="0" max="50000" step="500" value={minUsd} onChange={(e) => setMinUsd(Number(e.target.value))} />
        ...
        <div className="ctrl">
          Search
          <input type="text" placeholder="address / label" style={{ width: "150px" }} value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      ...
      <div className="stage" ref={stageRef} style={{ display: view === "graph" ? "block" : "none" }}>
        ...
        >
          {LAYERS.map((L) => (
            <g key={L}>
              ...
              <line x1={PAD_X + L * COL_W} y1={40} x2={PAD_X + L * COL_W} y2={GRAPH_H - 24} stroke="rgba(204,255,0,.09)" strokeWidth={1} />
            </g>
          ))}
          ...
          <g>
            {EDGES.map((e, idx) => {
              ...
              }<br>${ago(e.ts)} · ${e.sig}<span class="t-src ${e.flagged ? "own" : "arkham"}">${e.flagged ? "flagged path" : "normal"}</span>`;
              return (
                <g key={idx}>
                  <path d={d} className={`edge ${e.flagged ? "flagged" : "normal"} ${on ? "" : "dimmed"}`} strokeWidth={weight(e.usd)} />
                  <path d={d} className="edge-hit" onPointerEnter={(ev) => handleShowTip(ev, edgeTip)} onPointerLeave={handleHideTip} />
                </g>
              );
            })}
          </g>
          ...
          {NODES.map((n) => {
            ...
            const nodeTip = `<b>${n.label}</b><br>${n.addr}<br>${n.sub}<span class="t-src ${n.source === "gotham" ? "own" : "arkham"}">${
              n.source === "gotham" ? "Gotham engine" : "Arkham"
            }</span>`;
            return (
              <g
                key={n.id}
                className={`node ${n.flagged ? "flagged " : ""}${n.type === "entity" ? "entity " : ""}${focus === n.id ? "focus " : ""}${
                  isLive ? "" : "dimmed"
                }`}
        ...
        <div className={`tip ${tooltip.show ? "show" : ""}`} style={{ left: tooltip.x, top: tooltip.y }} dangerouslySetInnerHTML={{ __html: tooltip.html }} />

        <div className="legend">
          <span><i />Flagged path</span>
          <span><i className="dash" />Normal transfer</span>
          <span><i className="sq fill" />Flagged wallet</span>
          <span><i className="sq" />Clean / unknown</span>
      ...
      <div className="timeline">
        <div className="tl-head">
          ...
          <span>{tlStart === 0 && tlEnd === 1 ? "All 72 hours" : `−${h0}h → −${h1}h`}</span>
        </div>
        <div className="tl-bars" ref={tlBarsRef} onPointerDown={handleTlDown} onPointerMove={handleTlMove} onPointerUp={handleTlUp}>
          {vols.map((v, i) => {
            ...
            return <div key={i} className={`tl-bar ${flags[i] ? "flag" : ""} ${isOn ? "on" : ""}`} style={{ height: `${Math.max(2, (v / maxVol) * 46)}px` }} />;
          })}
        </div>
      ...
      <div className="panel">
        <div className="panel-head">
          ...
          <span className="count">{sortedRows.length} of {EDGES.length} transfers</span>
        </div>
        <div className="tbl-wrap">
          <table>
            <thead>
              <tr>
                {(["ts", "from", "to", "usd", "token", "dir", "hop", "source"] as const).map((k) => (
                  <th key={k} onClick={() => handleSort(k)} className={sortKey === k ? "sorted" : ""}>
                    {{ ts: "Time", from: "From", to: "To", usd: "Value", token: "Token", dir: "Dir", hop: "Hop", source: "Source" }[k]}{" "}
                    <span className="arw">{sortKey === k ? (sortDir === -1 ? "▾" : "▴") : "▾"}</span>
                  </th>
                ))}
                <th>Flag</th>
              </tr>
            ...
            <tbody>
              {sortedRows.length === 0 ? (
                <tr>
                  <td colSpan={9} className="empty">No transfers match these filters</td>
                </tr>
              ) : (
                sortedRows.map((e, idx) => {
                  ...
                  return (
                    <tr key={idx} className={e.flagged ? "flagged" : ""}>
                      <td>{ago(e.ts)}</td>
                      <td><span className="mono-b">{f?.label}</span> {f?.addr}</td>
                      <td><span className="mono-b">{t?.label}</span> {t?.addr}</td>
                      <td className="mono-b">{usdFmt(e.usd)}</td>
                      <td>{e.token}</td>
                      <td><span className={`tag dir-${e.dir}`}>{e.dir}</span></td>
                      <td>{e.hop}</td>
                      <td><span className={`tag ${src}`}>{src === "own" ? "Gotham engine" : "Arkham"}</span></td>
        ...
        <p className="note">
          Rows sourced by <em>Gotham engine</em> are read live from chain. Rows tagged <em>Arkham</em> carry an entity label resolved from their database.
          Click any node in the graph to filter this log to that wallet. <em>This panel is illustrative — scan an address above for a real result.</em>
        </p>
      </div>
    </div>
  );
