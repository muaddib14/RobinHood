...
- Wired as a tab ("Findings" / "Flow graph") next to the existing findings list — in both `ScanShell.tsx` (live scan) and the share page (`app/scan/[id]/ResultTabs.tsx`, extracted as its own client component since the share page itself is a server component and can't hold tab state directly).
